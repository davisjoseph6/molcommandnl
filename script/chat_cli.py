...
class ChatUI:
    ...
    def _update_panels(self, layout, debug_height, response_height):
        debug_lines_to_show = min(len(self.debug_lines), debug_height - 2)
        response_lines_to_show = min(len(self.response_lines), response_height - 2)
...
class DSLManager:
    ...
    def _load_dsl_module(self, dsl_name: str, key_name: str = None):
        ...
        try:
            ...
            DSLClass = load_dsl_from_path(dsl_path, f"{dsl_name}_dsl")
            ...
            try:
                ...
                print(f"[yellow]Warning: Could not load {dsl_name} DSL context file[/yellow]")

            TranspilerClass = load_transpiler_from_path(dsl_path, self.transpilers[dsl_name])
            transpiler_instance = TranspilerClass()

            return {
                key_name: {
                    "dsl": dsl_instance,
                    "interpreter": SemanticInterpreter(self.llm, dsl_instance),
...
def parse_arguments():
    ...
    parser.add_argument('--simple', action='store_true', 
                       help='Use simple linear chat interface')
...
def detect_dsl_type(query: str) -> str:
    ...
    if any(kw in query_lower for kw in ['pdb', 'protein', 'structure', 'molecule', 'load', 'display']):
        return "molcommand"
    if any(kw in query_lower for kw in ['slide', 'presentation', 'document', 'text', 'bold', 'italic', 'format']):
        return "office"
    if any(kw in query_lower for kw in ['markdown', 'heading', 'bullet', 'list', 'document', 'format']):
        return "markdown"
...
def process_query(query: str, dsls: dict, ui: ChatUI):
    ...
    try:
        ...
        ui.add_debug(f"[dim]DSL code generated:[/dim]")
        ui.add_debug(f"[yellow]{result['dsl']}[/yellow]")

        native_code = transpiler.transpile(result["ast"])
        ui.add_response(f"[bold green]✓ Success![/bold green] {dsl_type.upper()} result in [yellow]{elapsed_time:.2f} seconds[/yellow]:")

        if dsl_type == "molcommand":
            ui.add_response(f"[cyan]# IronPython Code:[/cyan]")
        elif dsl_type == "office":
            ui.add_response(f"[cyan]# JavaScript Code:[/cyan]")
        elif dsl_type == "markdown":
            ui.add_response(f"[cyan]# Markdown:[/cyan]")
        
        ui.add_response(f"[white]{native_code}[/white]")

    except Exception as e:
        ui.add_debug(f"[red]Error interpreting query: {str(e)}[/red]")
        ui.add_response(f"[bold red]Failed to process query.[/bold red]")
